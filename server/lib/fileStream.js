...
// 普通文件处理
const accessFile = (response, path, ContentType) => { 
...
// 多媒体文件处理
const accessImage = (response, path, ContentType) => {
    let responseData = [];
...
module.exports = {
    ...
    // 复制文件到新目录
    writeFileToPath : (tmpPath, path, callback) => {
        if(!tmpPath || !path) {
    ...
    // 原文件改名
    renameFile : (path, newPath, callback) => {
